...
public class ChatController {
    ...
    @GetMapping
    public ResponseEntity<List<AllChatsResponse>> getAllChats() {
        return ResponseEntity.ok(chatService.getAllChatsFromUser(getAuthenticatedEmail()));
    }

    @PostMapping("/create")
    public ResponseEntity<String> createChat(@RequestBody CreateChatRequest request) {
        chatService.createChat(request, getAuthenticatedEmail());
        return ResponseEntity.status(201).body("Chat créé!");
    }

    @PostMapping("/join")
    public ResponseEntity<String> joinChat(@RequestBody JoinChatRequest request) {
        chatService.joinChat(request, getAuthenticatedEmail());
    ...
    public ResponseEntity<String> updateChat(@PathVariable long chatId, @RequestBody UpdateChatRequest request) {
        chatService.updateChat(chatId, request, getAuthenticatedEmail());
    ...
    public ResponseEntity<String> removeMember(
        ...
        chatService.removeMember(chatId, memberId, getAuthenticatedEmail());
